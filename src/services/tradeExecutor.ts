...
const userActivityModels = USER_ADDRESSES.map((address: string) => ({
    address,
...
const logMidpointIfAvailable = async (tokenId: string, marketLabel?: string): Promise<void> => {
    try {
        const midpoint = await getPolymarketTokenMidpointPrice(tokenId);
        if (midpoint !== null) {
            Logger.info(`Midpoint (${marketLabel || tokenId}): $${midpoint.toFixed(4)}`);
        } else {
            Logger.warning(`Midpoint unavailable for ${marketLabel || tokenId}`);
        }
    } catch (error) {
        Logger.warning(
            `Midpoint fetch failed for ${marketLabel || tokenId}: ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
...
const doTrading = async (clobClient: ClobClient, trades: TradeWithUser[]) => {
    for (const trade of trades) {
        ...
        await logMidpointIfAvailable(trade.asset, trade.slug || trade.eventSlug);
...
const doAggregatedTrading = async (clobClient: ClobClient, aggregatedTrades: AggregatedTrade[]) => {
    for (const agg of aggregatedTrades) {
        ...
        await logMidpointIfAvailable(agg.asset, agg.slug || agg.eventSlug);
