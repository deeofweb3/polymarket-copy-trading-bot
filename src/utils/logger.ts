...
class Logger {
    ...
    private static writeToFile(message: string): void {
        try {
            ...
            // Intentionally ignore logging write failures to avoid recursive error logging.
        }
    }

    private static stripAnsi(str: string): string {
        // Strip ANSI color codes before writing plain-text logs to disk.
    ...
    private static maskAddress(address: string): string {
        // Preserve prefix/suffix for readability while masking sensitive address middle bytes.
    ...
    static trade(traderAddress: string, action: string, details: any) {
        ...
        if (details.eventSlug || details.slug) {
            // Prefer eventSlug because it maps to the canonical event URL format.
        ...
        // Persist a compact, searchable trade summary to the daily log file.
    ...
    static startup(traders: string[], myWallet: string) {
        console.log('\n');
        // Render a startup banner for operator visibility.
    ...
    static myPositions(
        ...
        // Print cash and portfolio totals before position-level details.
        ...
        if (count === 0) {
            ...
            // Print highest-value positions for quick monitoring.
    ...
    static tradersPositions(
        ...
        traders.forEach((address, index) => {
            ...
            // Include PnL summary when profitability data is available.
            let profitStr = '';
            ...
            // Print per-position details when they were fetched upstream.
