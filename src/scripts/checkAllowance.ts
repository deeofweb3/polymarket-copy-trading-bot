...
const syncPolymarketAllowanceCache = async (
    ...
    try {
        ...
        if (updateResult === '' || updateResult === null || updateResult === undefined) {
            ...
            logger.error(
                `⚠️  Polymarket cache update returned an unexpected response: ${JSON.stringify(updateResult)}`
            );
        } else {
        ...
        if (!balanceResponse || typeof balanceResponse !== 'object') {
            logger.error(
                `⚠️  Unexpected response from Polymarket when fetching balance/allowance: ${JSON.stringify(balanceResponse)}`
            );
            return;
        }
        ...
        if (balance === undefined || allowanceValue === undefined) {
            logger.error(
                `⚠️  Polymarket did not provide balance/allowance data. Raw response: ${JSON.stringify(balanceResponse)}`
            );
            return;
        }
