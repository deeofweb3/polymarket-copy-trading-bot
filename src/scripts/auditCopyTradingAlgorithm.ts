...
async function main() {
    ...
    try {
        ...
        logger.info(`📊 Analyzing ${traderAddresses.length} traders:\n`);
        traderAddresses.forEach((addr, idx) => {
            logger.info(`  ${idx + 1}. ${addr}`);
        });
        ...
        for (let i = 0; i < traderAddresses.length; i++) {
            ...
            if (!result.error && result.copiedTrades > 0) {
                ...
                logger.info(`  ${colors.yellow('⚠')} ${result.error || 'No trades copied'}`);
            }
