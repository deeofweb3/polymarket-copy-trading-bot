...
function parseTraderAddresses(): string[] {
    ...
    if (!envAddresses) {
        logger.warning('⚠️  No AUDIT_ADDRESSES or USER_ADDRESSES found in environment');
        logger.warning('    Using default test addresses...\n');
        return [
    ...
    if (envAddresses.trim().startsWith('[')) {
        try {
            ...
            logger.warning('⚠️  Failed to parse JSON array, trying comma-separated...');
        }
    }
...
async function fetchTraderActivity(traderAddress: string): Promise<Trade[]> {
    try {
        ...
        logger.error(`Error fetching trader ${traderAddress}: ${error}`);
        return [];
    }
}
...
async function simulateTrader(
    ...
    try {
        logger.info(`  Fetching trades for ${shortAddress}...`);
        ...
        logger.info(`  Simulating ${trades.length} trades for ${shortAddress}...`);
...
function printAuditReport(report: AuditReport) {
    ...
    logger.info('  🔍 COPY TRADING ALGORITHM AUDIT REPORT (FIXED)');
    logger.info(colors.cyan('═'.repeat(100)) + '\n');

    logger.info('📋 AUDIT CONFIGURATION:');
    logger.info(`  Period: ${colors.yellow(report.config.days + ' days')}`);
    logger.info(`  Multiplier: ${colors.yellow(report.config.multiplier + 'x')}`);
    logger.info(
    ...
    logger.info(`  Traders: ${colors.cyan(String(report.config.traders.length))}\n`);

    logger.info('👥 INDIVIDUAL TRADER RESULTS:\n');
    ...
    validResults.forEach((result, idx) => {
        ...
        logger.info(`${idx + 1}. ${colors.blue(result.shortAddress)}`);
        logger.info(
    ...
    if (errors.length > 0) {
        logger.info(colors.yellow(`⚠️  ${errors.length} traders had errors:\n`));
        errors.forEach((r) => {
            logger.info(`  • ${r.shortAddress} - ${colors.gray(r.error || 'Unknown error')}`);
        });
    }

    logger.info(colors.cyan('─'.repeat(100)) + '\n');
    logger.info('🤖 COMBINED BOT RESULT (copying all traders):\n');
    ...
    logger.info(`  Starting Capital: ${colors.green('$' + combined.startingCapital.toFixed(2))}`);
    logger.info(`  Current Capital:  ${colors.green('$' + combined.currentCapital.toFixed(2))}`);
    logger.info(
    ...
    logger.info(`  Realized P&L:     $${combined.realizedPnl.toFixed(2)}`);
    logger.info(`  Unrealized P&L:   $${combined.unrealizedPnl.toFixed(2)}`);
    logger.info(`  Win Rate:         ${combined.winRate.toFixed(1)}%`);
    logger.info(
    ...
    logger.info(`  Best Trader:               ${colors.green(analysis.bestTrader)}`);
    logger.info(`  Worst Trader:              ${colors.red(analysis.worstTrader)}`);
    logger.info(
    ...
    logger.info(colors.cyan('─'.repeat(100)) + '\n');
    logger.info('🎯 AUDIT CONCLUSIONS:\n');

    if (Math.abs(analysis.roiDeviation) < 1) {
        logger.info(`  ${colors.green('✓')} Algorithm working correctly (ROI deviation < 1%)`);
        ...
        logger.info(`      ${colors.yellow('→ Review algorithm logic for potential issues')}`);
    }
    ...
    if (analysis.diversificationBenefit > 5) {
        ...
        logger.info(`      ${colors.yellow('→ Consider trader selection criteria')}`);
    }
    ...
    if (combined.roi > 15) {
        ...
        logger.info(`  ${colors.yellow('○')} Good performance (${combined.roi.toFixed(2)}% ROI)`);
    } else if (combined.roi > 0) {
        logger.info(`  ${colors.yellow('○')} Modest performance (${combined.roi.toFixed(2)}% ROI)`);
    } else {
        logger.info(`  ${colors.red('✗')} Negative performance (${combined.roi.toFixed(2)}% ROI)`);
        logger.info(`      ${colors.yellow('→ Review trader selection and risk management')}`);
    }
...
async function main() {
    ...
    logger.info(colors.cyan('  🔍 INDEPENDENT AUDIT: COPY TRADING ALGORITHM VERIFICATION (FIXED)'));
    logger.info(colors.cyan('═'.repeat(100)) + '\n');

    logger.info('AUDIT PARAMETERS:');
    logger.info(`  History Period: ${colors.yellow(AUDIT_DAYS + ' days')}`);
    logger.info(`  Trade Multiplier: ${colors.yellow(AUDIT_MULTIPLIER + 'x')}`);
    logger.info(
        `  Copy Percentage: ${colors.yellow(COPY_PERCENTAGE + '%')} of trader's order size`
    );
    logger.info(`  Starting Capital: ${colors.green('$' + AUDIT_STARTING_CAPITAL.toFixed(2))}`);
    logger.info(`  Min Order Size: ${colors.yellow('$' + MIN_ORDER_SIZE.toFixed(2))}\n`);
