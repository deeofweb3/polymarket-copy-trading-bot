...
/**
 * Trim quotes/BOM/whitespace often introduced when pasting into .env files.
 */
 ...
 */
const isValidEvmPrivateKey = (hexNoPrefix: string): boolean => {
    if (!/^[a-fA-F0-9]{64}$/.test(hexNoPrefix)) {
        return false;
    }
    if (/^0{64}$/.test(hexNoPrefix)) {
        return false;
    }
    try {
        new ethers.Wallet(`0x${hexNoPrefix}`);
        return true;
    } catch {
        return false;
    }
...
const validatePrivateKey = (): void => {
    ...
    if (!/^[a-fA-F0-9]{64}$/.test(hex)) {
        ...
        logger.error(`Got hex length after normalization: ${hex.length}`);
        logger.error(
            '💡 Strip quotes/BOM in .env, use the raw key only (not your 0x address).\n'
        );
        throw new Error(
    ...
    if (/^0{64}$/.test(hex)) {
        logger.error('\n❌ Invalid PRIVATE_KEY — all-zero key is not allowed.\n');
        throw new Error(
            'Invalid PRIVATE_KEY: must be a valid Ethereum-compatible private key'
        );
    }
    if (!isValidEvmPrivateKey(hex)) {
        logger.error('\n❌ Invalid PRIVATE_KEY — bot startup aborted\n');
        logger.error('Expected: 64 hex chars forming a valid secp256k1 private scalar.');
        logger.error(`Hex length: ${hex.length} (format looks fine; cryptographic check failed).`);
        logger.error(
            '💡 Re-export the account key from your wallet. Placeholder/sample keys and some pasted strings can match length but fail validation.\n'
        );
        throw new Error(
            'Invalid PRIVATE_KEY: must be a valid Ethereum-compatible private key'
        );
...
const validateAddresses = (): void => {
    ...
    if (
        ...
        logger.error(`Current value: ${process.env.USDC_CONTRACT_ADDRESS}`);
...
const validateUrls = (): void => {
    if (process.env.CLOB_HTTP_URL && !process.env.CLOB_HTTP_URL.startsWith('http')) {
        ...
        logger.error(`Current value: ${process.env.CLOB_HTTP_URL}`);
        logger.error('Default value: https://clob.polymarket.com/\n');
        logger.error('⚠️  Use the default value unless you have a specific reason to change it!\n');
        throw new Error(
    ...
    if (process.env.CLOB_WS_URL && !process.env.CLOB_WS_URL.startsWith('ws')) {
        logger.error('\n❌ Invalid CLOB_WS_URL\n');
        logger.error(`Current value: ${process.env.CLOB_WS_URL}`);
        logger.error('Default value: wss://ws-subscriptions-clob.polymarket.com/ws\n');
        logger.error('⚠️  Use the default value unless you have a specific reason to change it!\n');
        throw new Error(
    ...
    if (process.env.RPC_URL && !process.env.RPC_URL.startsWith('http')) {
        logger.error('\n❌ Invalid RPC_URL\n');
        logger.error(`Current value: ${process.env.RPC_URL}`);
        logger.error('Must start with: http:// or https://\n');
        logger.error('💡 Get a free RPC endpoint from:');
        logger.error('   • Infura:  https://infura.io');
        logger.error('   • Alchemy: https://www.alchemy.com');
        logger.error('   • Ankr:    https://www.ankr.com\n');
        logger.error('Example: https://polygon-mainnet.infura.io/v3/YOUR_PROJECT_ID\n');
        throw new Error(`Invalid RPC_URL: ${process.env.RPC_URL}. Must be a valid HTTP/HTTPS URL.`);
    }

    if (process.env.MONGO_URI && !process.env.MONGO_URI.startsWith('mongodb')) {
        logger.error('\n❌ Invalid MONGO_URI\n');
        logger.error(`Current value: ${process.env.MONGO_URI}`);
        logger.error('Must start with: mongodb:// or mongodb+srv://\n');
        logger.error('💡 Setup MongoDB Atlas (free):');
        logger.error('   1. Visit https://www.mongodb.com/cloud/atlas/register');
        logger.error('   2. Create a free cluster');
        logger.error('   3. Create database user with password');
        logger.error('   4. Whitelist IP: 0.0.0.0/0 (or your IP)');
        logger.error('   5. Get connection string from "Connect" button\n');
        logger.error('Example: mongodb+srv://username:password@cluster.mongodb.net/database\n');
        throw new Error(
            ...
            if (Array.isArray(parsed)) {
                ...
                for (const addr of addresses) {
                    if (!isValidEthereumAddress(addr)) {
                        ...
                        logger.error('Expected format: 0x followed by 40 hexadecimal characters\n');
                        logger.error('💡 Where to find trader addresses:');
                        logger.error('   • Polymarket Leaderboard: https://polymarket.com/leaderboard');
                        logger.error('   • Predictfolio: https://predictfolio.com\n');
                        logger.error('Example: USER_ADDRESSES=\'0x7c3db723f1d4d8cb9c550095203b686cb11e5c6b\'\n');
...
const parseCopyStrategy = (): CopyStrategyConfig => {
    ...
    if (hasLegacyConfig) {
        ...
        if (process.env.TIERED_MULTIPLIERS) {
            try {
                ...
                logger.info(`✓ Loaded ${config.tieredMultipliers.length} tiered multipliers`);
            } catch (error) {
    ...
    if (process.env.TIERED_MULTIPLIERS) {
        ...
        if (singleMultiplier !== 1.0) {
            ...
            logger.info(`✓ Using single trade multiplier: ${singleMultiplier}x`);
        }
    }
